  ...
  const doc=new Document({
    ...
    sections:[{
      ...
      children:[
        ...
        sec(sigNum,"Signatures"),body("Lu et approuvé par les deux parties :"),empty(),empty(),
        new Table({
...
function Home({data,setData,setModule}){
  ...
  const ajouterProspect=()=>{
    ...
    const nouveau={
      id:data.nextId||100,nom:prospNom.trim(),instagram:prospInsta.trim(),
      plateforme:"Instagram",followers:"",engagement:"",univers:"Illustration",
      type:"Collab artistique",statut:"À contacter",dateContact:today(),
      delaiRelance:7,notes:"",contenus:[],deal:{},contrat:{notesSup:""},
    };
  ...
  return <div>
    ...
    {/* Ajout rapide prospect */}
    <div style={{background:T.bg1,border:`1px solid ${T.violet}33`,borderRadius:14,padding:"16px 18px",marginBottom:16,boxShadow:`0 0 20px ${T.violet}0a`}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {STATUTS_ACTIFS.map(s=>{
        ...
        return <div key={s} onClick={()=>setModule("collabs")} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:n>0?color+"11":T.bg1,border:`1px solid ${n>0?color+"44":T.border}`,borderRadius:10,cursor:"pointer",transition:"all 0.15s"}}>
  ...
  if(detail){
    ...
    return <div>
      ...
      <NeonCard accent={SACENT[c.statut]||T.violet} style={{marginBottom:12}}>
        ...
        {c.contenus?.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:c.notes?8:0}}>
  ...
  return <div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {filtered.map(c=>{
        ...
        return <div key={c.id} style={{background:n=>urgent?"#ff4db811":color+"11",background:urgent?"#ff4db811":color+"0a",border:`1px solid ${urgent?"#ff4db855":color+"44"}`,borderRadius:14,padding:"16px 18px"}}>
...
export default function App(){
  ...
  return <div style={{background:T.bg,minHeight:"100vh",color:T.text,fontFamily:"'DM Sans',sans-serif",maxWidth:480,margin:"0 auto",paddingBottom:90}}>
    ...
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:"rgba(8,8,8,0.97)",borderTop:`1px solid ${T.border}`,backdropFilter:"blur(20px)",padding:"10px 4px 24px",display:"flex",justifyContent:"space-around",zIndex:100}}>
      {MODULES.map(m=>{const active=module===m.id;const ic=active?m.iconColor:T.textDim;return <button key={m.id} onClick={()=>setModule(m.id)} style={{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"6px 20px",minWidth:80}}>
        <span style={{fontSize:20,color:ic,textShadow:active?`0 0 12px ${m.iconColor}`:""}}>{m.icon}</span>
        <span style={{fontSize:9,color:active?m.iconColor:"#333",fontWeight:active?700:400,letterSpacing:0.5,textTransform:"uppercase"}}>{m.label}</span>
