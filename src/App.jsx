...
const SACENT = {
  "À contacter":T.violet,"En attente de réponse":T.amber,"Relancer":"#ff4db8",
  "Collab confirmée":T.green,"Drop en cours":T.cyan,"Terminé":T.textMid,"Archivé":T.textDim,
};
...
// ── TEMPLATES MESSAGES ────────────────────────────────────────────
const buildDM1 = (prenom, univers) => {
  const salut = prenom ? `Bonjour ${prenom} 🧪` : "Bonjour 🧪";
  const univLine = univers
    ? `votre univers autour de ${univers} m'a tout de suite parlé`
    : "votre univers m'a tout de suite parlé";
  return `${salut}

Je m'appelle Florian, je suis à la tête de StickersLab, on conçoit des packs de stickers en édition limitée avec des créateurs.

J'ai regardé votre contenu et honnêtement, ${univLine}. Je pense qu'on pourrait faire quelque chose de sympa ensemble.

Concrètement, la collaboration est entièrement gratuite pour vous. On s'occupe de tout : le design, l'impression, la vente et les envois. Votre seule implication c'est de présenter le pack à votre communauté pendant 2 semaines. En retour, vous touchez un pourcentage sur chaque pack vendu.

Vous seriez partant(e) pour qu'on en parle ?`;
};

const buildDM7 = (prenom) => {
  const salut = prenom ? `Bonjour ${prenom} 🧪` : "Bonjour 🧪";
  return `${salut}

Je me permets de revenir vers vous, je ne sais pas si mon message vous a atteint.

Je reste convaincu que votre univers colle parfaitement à ce qu'on fait chez StickersLab. La proposition tient toujours : collaboration entièrement gratuite, on gère tout de A à Z.

Si ce n'est pas le bon moment ou que ça ne vous correspond pas, pas de souci du tout. Mais si vous êtes curieux(se), je suis dispo pour vous expliquer ça en quelques lignes.

Bonne journée à vous.`;
};

const buildDM14 = (prenom) => {
  const salut = prenom ? `Bonjour ${prenom} 🧪` : "Bonjour 🧪";
  return `${salut}

C'est mon dernier message, je ne veux pas vous encombrer.

Je garde votre profil de côté car je pense sincèrement qu'il y a quelque chose à faire ensemble. Si un jour la curiosité vous prend, n'hésitez pas à revenir vers moi.

Belle continuation à vous.`;
};

const buildEmail = (prenom, univers) => {
  const salut = prenom ? `Bonjour ${prenom},` : "Bonjour,";
  const univLine = univers
    ? `votre univers autour de ${univers} m'a immédiatement interpellé`
    : "votre univers m'a immédiatement interpellé";
  return `${salut}

Je m'appelle Florian, je suis fondateur de StickersLab, une marque spécialisée dans la création de packs de stickers en édition limitée, conçus en collaboration avec des créateurs de contenu.

J'ai découvert votre travail et ${univLine}. Je pense sincèrement qu'une collaboration entre nous aurait beaucoup de sens.

Voici comment ça fonctionne concrètement :

La collaboration est entièrement gratuite pour vous. Nous prenons en charge la totalité du processus : conception graphique des stickers en lien avec votre identité visuelle, impression, gestion des ventes et expédition des commandes. De votre côté, la seule implication attendue est de présenter le pack à votre communauté pendant 2 semaines. En échange, vous percevez un pourcentage sur chaque pack vendu.

L'objectif est simple : créer ensemble un produit qui vous ressemble et que votre audience aura envie de s'approprier.

Si cela vous intéresse, je suis disponible pour en discuter à votre convenance.

Cordialement,
Florian
StickersLab. | Collabstickerslab@gmail.com`;
};

const buildEmailSujet = (prenom) =>
  prenom ? `Collaboration stickers x ${prenom} 🧪` : "Collaboration stickers 🧪";

const TEMPLATES_LIST = [
  { id:"dm1",   titre:"DM — 1er contact",        icone:"✉️", couleur:T.violet, hasUnivers:true  },
  { id:"dm7",   titre:"DM — Relance J+7",         icone:"🔔", couleur:T.amber,  hasUnivers:false },
  { id:"dm14",  titre:"DM — Dernier message J+14",icone:"👋", couleur:T.textMid,hasUnivers:false },
  { id:"email", titre:"Email — 1er contact",      icone:"📧", couleur:T.green,  hasUnivers:true, hasSujet:true },
  { id:"contrat",titre:"Contrat de collaboration",icone:"📄", couleur:T.cyan,   isContrat:true   },
];

// ── WORD GENERATOR ────────────────────────────────────────────────
async function genWordContrat(fields) {
  const {
    nom="", plateforme="", instagram="", email="",
    pourcentage="30", dateDebut="", duree="", notesSup=""
  } = fields;

  const pct = parseFloat(pourcentage||30);
  const partSL = 100 - pct;
  ...
  const fld=(label,val)=>new Paragraph({spacing:sp,children:[new TextRun({text:`${label} : `,bold:true,size:20,font:"Arial"}),new TextRun({text:val||"",size:20,font:"Arial"})]});
  ...
  const hasSup = notesSup.trim().length > 0;
  ...
  const doc = new Document({
    ...
    sections:[{
      ...
      children:[
        ttl("STICKERS LAB"),
        sub("Contrat de collaboration — Édition de stickers personnalisés"),
        hr(), empty(),
        ...
        fld("Nom / Pseudo", nom),
        fld("Plateforme", plateforme),
        fld("URL de la page", instagram),
        fld("Email", email),
        empty(),
        ...
        fld("Représenté par","M. Besnard Florian"),
        fld("Email","Collabstickerslab@gmail.com"),
        empty(), hr(),
        ...
        new Table({
          ...
          rows:[
            new TableRow({tableHeader:true,children:[
              ...
              new TableCell({borders,width:{size:2000,type:WidthType.DXA},shading:{fill:"222222",type:ShadingType.CLEAR},margins:{top:80,bottom:80,left:120,right:120},children:[new Paragraph({alignment:AlignmentType.CENTER,children:[new TextRun({text:"Part des revenus",bold:true,size:20,font:"Arial",color:"FFFFFF"})]})]})  ,
        ...
        empty(),
        body("Toute modification demandée après validation et mise en production pourra entraîner des frais supplémentaires."),
        empty(), hr(),
        ...
        fld("Date de début", dateDebut),
        fld("Durée initiale", duree),
        empty(),
        ...
        ...(hasSup ? [
          sec("10","Conditions particulières"),
          body(notesSup),
          empty(), hr(),
        ] : []),
        ...
        new Table({
          ...
          rows:[
            ...
            new TableRow({children:[
              new TableCell({borders:noBorders,width:{size:4513,type:WidthType.DXA},margins:{top:80,bottom:80,left:0,right:120},children:[fld("Nom",nom)]}),
            ...
            new TableRow({children:[
              new TableCell({borders:noBorders,width:{size:4513,type:WidthType.DXA},margins:{top:80,bottom:80,left:0,right:120},children:[fld("Date","")]}),
              new TableCell({borders:noBorders,width:{size:4513,type:WidthType.DXA},margins:{top:80,bottom:80,left:120,right:0},children:[fld("Date","")]}),
            ]}),
            new TableRow({children:[
        ...
        empty(), empty(),
        new Paragraph({alignment:AlignmentType.CENTER,spacing:{before:240,after:0},children:[new TextRun({text:`Document généré par StickersLab. — ${new Date().toLocaleDateString("fr-FR")}`,size:16,font:"Arial",color:"999999",italics:true})]}),
      ]
    }]
  });
  ...
  a.href = url;
  a.download = `Contrat_${(nom||"Partenaire").replace(/\s+/g,"_")}_StickersLab.docx`;
  a.click();
...
function StatCard({label,value,accent}){
  const c=accent||T.green;
  return <div style={{background:T.bg1,border:`1px solid ${c}33`,borderRadius:14,padding:"14px 16px",flex:1}}><div style={{fontSize:20,fontWeight:800,color:c,fontFamily:"'DM Mono',monospace",letterSpacing:-1}}>{value}</div><div style={{fontSize:10,color:T.textDim,textTransform:"uppercase",letterSpacing:2,marginTop:3}}>{label}</div></div>;
}
...
function Btn({children,onClick,variant="primary",small,loading}){
  const S={primary:{bg:T.green,border:"none",color:"#000",fw:700},violet:{bg:T.violet,border:"none",color:"#fff",fw:700},ghost:{bg:"transparent",border:`1px solid ${T.border2}`,color:T.textMid,fw:400},danger:{bg:"transparent",border:`1px solid ${T.danger}44`,color:T.danger,fw:400},cyan:{bg:T.cyan,border:"none",color:"#000",fw:700}};
...
function Logo(){
  return <div style={{display:"flex",alignItems:"center",gap:12}}>
    <img src="/logo.png" alt="StickersLab." onError={e=>{e.target.style.display="none";e.target.nextSibling.style.display="flex";}} style={{width:40,height:40,objectFit:"contain",borderRadius:10}}/>
    <div style={{display:"none",width:40,height:40,borderRadius:10,background:`linear-gradient(135deg,${T.violet},${T.green})`,alignItems:"center",justifyContent:"center",fontSize:20,boxShadow:`0 0 16px ${T.green}55`}}>⬡</div>
    <div>
      <div style={{fontSize:22,fontWeight:800,color:T.text,letterSpacing:-0.5,lineHeight:1}}>StickersLab<span style={{color:T.green}}>.</span></div>
      <div style={{fontSize:10,color:T.textDim,letterSpacing:2,textTransform:"uppercase"}}>Business Hub</div>
    </div>
  </div>;
}
...
const DEF = {
  collabs:[
    {id:1,nom:"LucieDraws",instagram:"https://instagram.com/luciedraws",plateforme:"Instagram",followers:"45k",engagement:"4.8",univers:"Illustration",type:"Collab artistique",statut:"Collab confirmée",dateContact:today(),delaiRelance:7,notes:"Fan de stickers",contenus:["Art","Lifestyle"],deal:{prix:"4.50",quantite:"100",pourcentage:"30"},contrat:{notesSup:""}},
    {id:2,nom:"PixelTom",instagram:"https://instagram.com/pixeltom",plateforme:"Instagram",followers:"80k",engagement:"3.2",univers:"Gaming",type:"Collab artistique",statut:"En attente de réponse",dateContact:today(),delaiRelance:7,notes:"",contenus:["Gaming"],deal:{},contrat:{notesSup:""}},
  ],
...
const MODULES = [
  ...
  { id:"docs",    icon:"📋", label:"Templates", iconColor:T.cyan   },
];
...
function Docs(){
  ...
  const [cPlateforme,setCPlateforme]=useState("Instagram");
  const [cInstagram,setCInstagram]=useState("");
  const [cEmail,setCEmail]=useState("");
  const [cPct,setCPct]=useState("30");
  ...
  const getTexte = () => {
    if(!selected) return "";
    if(selected.id==="dm1")   return buildDM1(prenom.trim(), univers.trim());
    if(selected.id==="dm7")   return buildDM7(prenom.trim());
    if(selected.id==="dm14")  return buildDM14(prenom.trim());
    if(selected.id==="email") return buildEmail(prenom.trim(), univers.trim());
    return "";
  };
  ...
  const handleGenContrat = async () => {
    setGenLoading(true);
    try {
      await genWordContrat({nom:cNom,plateforme:cPlateforme,instagram:cInstagram,email:cEmail,pourcentage:cPct,dateDebut:cDateDebut,duree:cDuree,notesSup:cNotes});
  ...
  if(!selected) return <div>
    <div style={{marginBottom:20}}>
      ...
      <div style={{fontSize:22,fontWeight:800,color:T.text}}>Documents <span style={{color:T.cyan}}>&amp; Templates</span></div>
    </div>
    <div style={{fontSize:12,color:T.textMid,marginBottom:20,lineHeight:1.6}}>Sélectionne un template, remplis les champs et copie en un clic.</div>
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {TEMPLATES_LIST.map(t=>(
        <div key={t.id} onClick={()=>{setSelected(t);setPrenom("");setUnivers("");setCopied(false);}} style={{background:T.bg1,border:`1px solid ${t.couleur}33`,borderRadius:14,padding:"16px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:14}} onMouseEnter={e=>e.currentTarget.style.borderColor=t.couleur+"66"} onMouseLeave={e=>e.currentTarget.style.borderColor=t.couleur+"33"}>
          <span style={{fontSize:24}}>{t.icone}</span>
          <div style={{flex:1}}>
            <div style={{fontWeight:700,fontSize:14,color:T.text,marginBottom:3}}>{t.titre}</div>
            <div style={{fontSize:11,color:T.textDim}}>{t.isContrat?"Remplis les champs et télécharge le .docx":"Personnalise et copie en un clic"}</div>
          </div>
          <span style={{color:t.couleur,fontSize:16}}>→</span>
        </div>
      ))}
    </div>
  </div>;
  ...
  // ── VUE CONTRAT ──────────────────────────────────────────────────
  if(selected.isContrat) return <div>
    <button onClick={()=>setSelected(null)} style={{background:"none",border:"none",color:T.cyan,cursor:"pointer",fontSize:13,marginBottom:16,padding:0,display:"flex",alignItems:"center",gap:6}}>← Templates</button>
    <NeonCard accent={T.cyan} style={{marginBottom:20}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
        <span style={{fontSize:20}}>📄</span>
        <div style={{fontSize:15,fontWeight:700,color:T.text}}>Contrat de collaboration</div>
      </div>
      <div style={{fontSize:11,color:T.textDim}}>Remplis les champs puis télécharge le contrat Word officiel</div>
    </NeonCard>
    ...
    <Input label="Nom / Pseudo" value={cNom} onChange={setCNom} placeholder="ex: LucieDraws"/>
    <Select label="Plateforme" value={cPlateforme} onChange={setCPlateforme} options={PLATEFORMES}/>
    <Input label="URL de la page" value={cInstagram} onChange={setCInstagram} placeholder="ex: instagram.com/luciedraws"/>
    <Input label="Email du créateur" value={cEmail} onChange={setCEmail} placeholder="ex: lucie@email.com"/>
    ...
    <Input label="Part créateur (%)" value={cPct} onChange={setCPct} type="number" placeholder="30"/>
    {cPct&&<div style={{background:T.greenDim,border:`1px solid ${T.green}33`,borderRadius:8,padding:"10px 14px",marginBottom:14,display:"flex",gap:20}}>
      <div><div style={{fontSize:10,color:T.textDim}}>Créateur</div><div style={{fontSize:15,fontWeight:700,color:T.green,fontFamily:"'DM Mono',monospace"}}>{cPct}%</div></div>
      <div><div style={{fontSize:10,color:T.textDim}}>StickersLab</div><div style={{fontSize:15,fontWeight:700,color:T.violet,fontFamily:"'DM Mono',monospace"}}>{100-parseFloat(cPct||0)}%</div></div>
    </div>}

    <div style={{fontSize:10,color:T.violet,letterSpacing:2,textTransform:"uppercase",marginBottom:12,marginTop:8,paddingBottom:8,borderBottom:`1px solid ${T.border}`}}>Durée</div>
    <Input label="Date de début" value={cDateDebut} onChange={setCDateDebut} type="date"/>
    <Input label="Durée initiale" value={cDuree} onChange={setCDuree} placeholder="ex: jusqu'à la clôture du drop"/>
    ...
    <Textarea label="" value={cNotes} onChange={setCNotes} placeholder="Ex : le créateur livrera 3 visuels minimum..." rows={3}/>

    <Btn variant="cyan" loading={genLoading} onClick={handleGenContrat}>⬇ Télécharger le contrat .docx</Btn>
  </div>;

  // ── VUE MESSAGE ──────────────────────────────────────────────────
  const texte = getTexte();
  const sujet = selected.hasSujet ? buildEmailSujet(prenom.trim()) : "";

  return <div>
    <button onClick={()=>setSelected(null)} style={{background:"none",border:"none",color:T.cyan,cursor:"pointer",fontSize:13,marginBottom:16,padding:0,display:"flex",alignItems:"center",gap:6}}>← Templates</button>

    <NeonCard accent={selected.couleur} style={{marginBottom:16}}>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:4}}>
        <span style={{fontSize:20}}>{selected.icone}</span>
        <div style={{fontSize:15,fontWeight:700,color:T.text}}>{selected.titre}</div>
      </div>
      <div style={{fontSize:11,color:T.textDim}}>Champs optionnels — laisse vide si tu ne veux pas les inclure</div>
    </NeonCard>

    <div style={{display:"flex",gap:10,marginBottom:16}}>
      <div style={{flex:1}}><Input label="Prénom" value={prenom} onChange={setPrenom} placeholder="ex: Sarah"/></div>
      {selected.hasUnivers&&<div style={{flex:1}}><Input label="Univers" value={univers} onChange={setUnivers} placeholder="ex: la motivation"/></div>}
    </div>

    {sujet&&<div style={{background:T.bg1,border:`1px solid ${T.border2}`,borderRadius:10,padding:"10px 14px",marginBottom:12}}>
      <div style={{fontSize:10,color:T.textDim,letterSpacing:2,textTransform:"uppercase",marginBottom:4}}>Objet email</div>
      <div style={{fontSize:13,color:T.text,fontWeight:500}}>{sujet}</div>
    </div>}

    <div style={{background:T.bg1,border:`1px solid ${selected.couleur}33`,borderRadius:12,padding:"16px",marginBottom:16}}>
      <div style={{fontSize:10,color:T.textDim,letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>Aperçu</div>
      <div style={{fontSize:13,color:T.textMid,lineHeight:1.8,whiteSpace:"pre-wrap"}}>{texte}</div>
    </div>

    <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
      <Btn variant="cyan" onClick={()=>copyText(sujet?`Objet : ${sujet}\n\n${texte}`:texte)}>
        {copied?"✓ Copié !":"📋 Copier"}
      </Btn>
      {sujet&&<Btn variant="ghost" small onClick={()=>copyText(sujet)}>Copier l'objet seul</Btn>}
    </div>
  </div>;
}

// ── MODULE: HOME ──────────────────────────────────────────────────
function Home({data,setData,setModule}){
  const [prospNom,setProspNom]=useState("");
  const [prospInsta,setProspInsta]=useState("");
  ...
  const aRelancer=collabs.filter(c=>{if(c.statut!=="En attente de réponse")return false;return today()>=addDays(c.dateContact,c.delaiRelance||7);}).length;
  ...
  const ajouterProspect=()=>{
    if(!prospNom.trim())return;
    const nouveau={id:data.nextId||100,nom:prospNom.trim(),instagram:prospInsta.trim(),plateforme:"Instagram",followers:"",engagement:"",univers:"Illustration",type:"Collab artistique",statut:"À contacter",dateContact:today(),delaiRelance:7,notes:"",contenus:[],deal:{},contrat:{notesSup:""}};
    setData(d=>({...d,collabs:[...(d.collabs||[]),nouveau],nextId:(d.nextId||100)+1}));
    setProspNom("");setProspInsta("");setShowProspForm(false);
  };

  return <div>
    <div style={{marginBottom:24}}><Logo/></div>
    <div style={{background:T.bg1,border:`1px solid ${T.violet}33`,borderRadius:14,padding:"16px 18px",marginBottom:16}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:showProspForm?14:0}}>
        <div>
          <div style={{fontSize:13,fontWeight:700,color:T.text}}>🔍 Nouveau prospect</div>
          {!showProspForm&&<div style={{fontSize:11,color:T.textDim,marginTop:2}}>Ajoute un Instagram à contacter</div>}
        </div>
        <button onClick={()=>setShowProspForm(v=>!v)} style={{background:showProspForm?T.border2:T.violet,border:"none",borderRadius:8,padding:"7px 14px",color:"#fff",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>
          {showProspForm?"Annuler":"+ Ajouter"}
        </button>
      </div>
      {showProspForm&&<>
        <Input label="Nom / Pseudo" value={prospNom} onChange={setProspNom} placeholder="ex: PixelTom"/>
        <Input label="Lien Instagram" value={prospInsta} onChange={setProspInsta} placeholder="https://instagram.com/pixeltom"/>
        <Btn variant="violet" onClick={ajouterProspect}>Ajouter dans "À contacter"</Btn>
      </>}
    </div>

    <a href={MY_INSTAGRAM} target="_blank" rel="noreferrer" style={{display:"flex",alignItems:"center",gap:10,background:"#E1306C11",border:"1px solid #E1306C33",borderRadius:12,padding:"12px 16px",textDecoration:"none",marginBottom:16}}>
      <span style={{fontSize:20}}>📷</span>
      <div><div style={{fontSize:13,fontWeight:700,color:"#E1306C"}}>Mon Instagram</div><div style={{fontSize:11,color:T.textDim}}>@stickerslab.officiel</div></div>
    ...
    {objectif>0&&<NeonCard accent={T.green} style={{marginBottom:16}}>
    ...
    <div style={{fontSize:10,color:T.textDim,letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>Pipeline</div>
    <div style={{display:"flex",flexDirection:"column",gap:6}}>
      {STATUTS_ACTIFS.map(s=>{
        const n=collabs.filter(c=>c.statut===s).length;
        const color=SACENT[s]||T.textDim;
        return <div key={s} onClick={()=>setModule("collabs")} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:n>0?color+"11":T.bg1,border:`1px solid ${n>0?color+"44":T.border}`,borderRadius:10,cursor:"pointer"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <span style={{width:8,height:8,borderRadius:"50%",background:n>0?color:T.border2,display:"inline-block",boxShadow:n>0?`0 0 6px ${color}`:""}}/>
            <span style={{fontSize:13,color:n>0?T.text:T.textDim,fontWeight:n>0?500:400}}>{s}</span>
          </div>
          <span style={{fontSize:14,fontWeight:700,color:n>0?color:T.textDim,fontFamily:"'DM Mono',monospace",textShadow:n>0?`0 0 8px ${color}33`:""}}>{n}</span>
        </div>;
      })}
    </div>
  </div>;
...
function Collabs({data,setData}){
  ...
  const [genLoading,setGenLoading]=useState(false);
  const collabs=data.collabs||[];
  const aRelancer=collabs.filter(c=>{if(c.statut!=="En attente de réponse")return false;return today()>=addDays(c.dateContact,c.delaiRelance||7);});
  const filtered=filter==="Tous"?collabs.filter(c=>c.statut!=="Archivé"):collabs.filter(c=>c.statut===filter);
  ...
  const save=form=>{setData(d=>{const id=form.id||d.nextId;const collabs=form.id?d.collabs.map(x=>x.id===form.id?{...form}:x):[...(d.collabs||[]),{...form,id}];return {...d,collabs,nextId:form.id?d.nextId:d.nextId+1};});setModal(null);if(detail&&form.id===detail.id)setDetail({...form});};
  const del=id=>{if(!confirm("Supprimer ?"))return;setData(d=>({...d,collabs:(d.collabs||[]).filter(x=>x.id!==id)}));setDetail(null);};
  const nextStatut=c=>{const next=STATUT_NEXT[c.statut]||c.statut;const updated={...c,statut:next};setData(d=>({...d,collabs:d.collabs.map(x=>x.id===c.id?updated:x)}));if(detail&&detail.id===c.id)setDetail(updated);};
  const downloadContrat=async c=>{
    setGenLoading(true);
    try{
      await genWordContrat({nom:c.nom,plateforme:c.plateforme,instagram:c.instagram,email:c.emailCreateur||"",pourcentage:c.deal?.pourcentage||"30",dateDebut:"",duree:"",notesSup:c.contrat?.notesSup||""});
    }catch(e){alert("Erreur : "+e.message);}
    setGenLoading(false);
  };
    ...
    return <div>
      ...
      <NeonCard accent={SACENT[c.statut]||T.violet} style={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
          <div><div style={{fontSize:20,fontWeight:800,color:T.text,marginBottom:4}}>{c.nom}</div><div style={{fontSize:11,color:T.textDim}}>{c.plateforme}{c.followers?` · ${c.followers} followers`:""}</div></div>
          <Badge label={c.statut}/>
        </div>
        {c.instagram&&<a href={c.instagram.startsWith("http")?c.instagram:`https://${c.instagram}`} target="_blank" rel="noreferrer" style={{display:"inline-flex",alignItems:"center",gap:6,background:"#E1306C22",border:"1px solid #E1306C44",borderRadius:8,padding:"6px 12px",fontSize:12,color:"#E1306C",textDecoration:"none",marginBottom:10}}>📷 Ouvrir Instagram</a>}
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:(c.contenus?.length||c.notes)?8:0}}>
        ...
        {c.contenus?.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:c.notes?8:0}}>{c.contenus.map(ct=><span key={ct} style={{fontSize:10,background:T.violetDim,border:`1px solid ${T.violet}33`,color:T.violet,borderRadius:6,padding:"2px 8px"}}>{ct}</span>)}</div>}
      ...
      {c.statut==="En attente de réponse"&&(()=>{const relanceDate=addDays(c.dateContact,c.delaiRelance||7);const diff=daysDiff(relanceDate);const urgent=diff<=0;const bientot=diff>0&&diff<=2;return <div style={{background:urgent?"#ff4db811":bientot?"#ffaa0011":T.bg1,border:`1px solid ${urgent?"#ff4db833":bientot?"#ffaa0033":T.border}`,borderRadius:10,padding:"10px 14px",marginBottom:12}}><div style={{fontSize:12,color:urgent?"#ff4db8":bientot?T.amber:T.textMid}}>{urgent?`🔴 Relance due depuis ${Math.abs(diff)}j`:bientot?`🟡 Relance dans ${diff}j`:`Relance prévue le ${fmtDateShort(relanceDate)}`}</div></div>;})()}

      {isConfirmed(c)&&<>
        <div style={{fontSize:10,color:T.textDim,letterSpacing:2,textTransform:"uppercase",marginBottom:10}}>💰 Le Deal — collab avec {c.nom}</div>
        <NeonCard accent={T.green} style={{marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:12,paddingBottom:12,borderBottom:`1px solid ${T.border}`}}>
            ...
            <div><div style={{fontSize:13,fontWeight:700,color:T.text}}>{c.nom}</div>{c.instagram&&<a href={c.instagram.startsWith("http")?c.instagram:`https://${c.instagram}`} target="_blank" rel="noreferrer" style={{fontSize:11,color:"#E1306C",textDecoration:"none"}}>📷 {c.instagram.replace("https://","").replace("http://","")}</a>}</div>
          </div>
          {deal.prix||deal.quantite?<>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:10}}>
              {[{l:"Prix du pack",v:deal.prix?`${deal.prix}€`:"—"},{l:"Nb de packs",v:deal.quantite||"—"},{l:"Part créateur",v:deal.pourcentage?`${deal.pourcentage}%`:"—"}].map(({l,v})=>(
                <div key={l} style={{background:T.bg,borderRadius:8,padding:"10px"}}><div style={{fontSize:9,color:T.textDim,marginBottom:3,letterSpacing:1}}>{l}</div><div style={{fontSize:14,fontWeight:700,color:T.text,fontFamily:"'DM Mono',monospace"}}>{v}</div></div>
              ))}
            </div>
            {revEstime&&<div style={{borderTop:`1px solid ${T.border}`,paddingTop:10,display:"flex",gap:16}}>
              <div><div style={{fontSize:9,color:T.textDim,marginBottom:3,letterSpacing:1}}>TOTAL</div><div style={{fontSize:14,fontWeight:700,color:T.text,fontFamily:"'DM Mono',monospace"}}>{revEstime}€</div></div>
  ...
  return <div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
      <div><div style={{fontSize:10,color:T.textDim,letterSpacing:3,textTransform:"uppercase"}}>Module</div><div style={{fontSize:22,fontWeight:800,color:T.text}}>Collabs <span style={{color:T.violet}}>&amp; Contrats</span></div></div>
    ...
    {aRelancer.length>0&&<div style={{background:"#ff4db822",border:"1px solid #ff4db855",borderRadius:12,padding:"12px 16px",marginBottom:16,display:"flex",gap:10,alignItems:"center"}}>
      <span>🔔</span><div style={{flex:1}}><div style={{fontSize:13,fontWeight:700,color:"#ff4db8"}}>{aRelancer.length} à relancer</div></div>
    </div>}

    <div style={{marginBottom:18}}>
      <select value={filter} onChange={e=>setFilter(e.target.value)} style={{width:"100%",background:T.bg1,border:`1px solid ${T.violet}44`,borderRadius:10,padding:"11px 14px",color:T.text,fontSize:13,outline:"none",fontFamily:"inherit",fontWeight:500,cursor:"pointer"}}>
        <option value="Tous">Tous les statuts (hors archivés)</option>
        {STATUTS.map(s=><option key={s} value={s}>{s} ({collabs.filter(c=>c.statut===s).length})</option>)}
      </select>
    </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:10}}>
      {filtered.map(c=>{
        ...
        return <div key={c.id} style={{background:urgent?"#ff4db811":color+"0a",border:`1px solid ${urgent?"#ff4db855":color+"44"}`,borderRadius:14,padding:"16px 18px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10,cursor:"pointer"}} onClick={()=>setDetail(c)}>
            <div>
              ...
              <div style={{fontSize:11,color:T.textDim,display:"flex",alignItems:"center",gap:6}}>{c.plateforme}{c.followers?` · ${c.followers}`:""}{c.engagement&&<span style={{color:engColor(c.engagement)}}>· ◈ {c.engagement}%</span>}</div>
            </div>
            <Badge label={c.statut}/>
          </div>
          {c.contenus?.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>{c.contenus.map(ct=><span key={ct} style={{fontSize:10,background:T.violetDim,border:`1px solid ${T.violet}33`,color:T.violet,borderRadius:6,padding:"2px 8px"}}>{ct}</span>)}</div>}
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            {c.statut!=="Archivé"&&<button onClick={e=>{e.stopPropagation();nextStatut(c);}} style={{background:color+"22",border:`1px solid ${color}55`,borderRadius:8,padding:"7px 12px",color:color,fontSize:11,cursor:"pointer",fontFamily:"inherit",fontWeight:600,whiteSpace:"nowrap",textShadow:`0 0 8px ${color}55`}}>→ {STATUT_NEXT[c.statut]}</button>}
          </div>
        </div>;
      })}
    ...
    {modal&&<Modal title={modal.id?"Modifier":"Nouvelle collab"} onClose={()=>setModal(null)}>
      ...
      {isConfirmed(modal)&&<>
        ...
        {modal.deal?.prix&&modal.deal?.quantite&&modal.deal?.pourcentage&&(()=>{const rev=Math.round(parseFloat(modal.deal.prix)*parseInt(modal.deal.quantite));const gc=Math.round(rev*parseFloat(modal.deal.pourcentage)/100);return <div style={{background:T.greenDim,border:`1px solid ${T.green}33`,borderRadius:8,padding:"10px 14px",marginBottom:14,display:"flex",gap:16}}><div><div style={{fontSize:10,color:T.textDim}}>Total</div><div style={{fontSize:15,fontWeight:700,color:T.text,fontFamily:"'DM Mono',monospace"}}>{rev}€</div></div><div><div style={{fontSize:10,color:T.textDim}}>Créateur</div><div style={{fontSize:15,fontWeight:700,color:T.green,fontFamily:"'DM Mono',monospace"}}>{gc}€</div></div><div><div style={{fontSize:10,color:T.textDim}}>StickersLab</div><div style={{fontSize:15,fontWeight:700,color:T.violet,fontFamily:"'DM Mono',monospace"}}>{rev-gc}€</div></div></div>;})()}
...
function Finance({data,setData}){
  ...
  return <div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
      {collabs.filter(c=>["Collab confirmée","Drop en cours","Terminé"].includes(c.statut)).map(c=>{const d=c.deal||{};const rev=Math.round(parseFloat(d.prix||0)*parseInt(d.quantite||0));const gc=d.pourcentage?Math.round(rev*parseFloat(d.pourcentage)/100):null;return <div key={c.id} style={{background:T.bg1,border:`1px solid ${T.border}`,borderRadius:12,padding:"14px 16px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6}}>{[{l:"Packs",v:d.quantite||"—"},{l:"Total estimé",v:rev>0?fmtEur(rev):"—"},{l:"Créateur",v:gc?fmtEur(gc):"—",col:T.green}].map(({l,v,col})=>(
...
export default function App(){
  ...
  const render=()=>{
    switch(module){
      case "home":    return <Home data={data} setData={setData} setModule={setModule}/>;
      case "collabs": return <Collabs data={data} setData={setData}/>;
      case "finance": return <Finance data={data} setData={setData}/>;
      case "docs":    return <Docs/>;
      default:        return <Home data={data} setData={setData} setModule={setModule}/>;
    }
  };
  return <div style={{background:T.bg,minHeight:"100vh",color:T.text,fontFamily:"'DM Sans',sans-serif",maxWidth:480,margin:"0 auto",paddingBottom:90}}>
    ...
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:480,background:"rgba(8,8,8,0.97)",borderTop:`1px solid ${T.border}`,backdropFilter:"blur(20px)",padding:"10px 4px 24px",display:"flex",justifyContent:"space-around",zIndex:100}}>
      {MODULES.map(m=>{const active=module===m.id;const ic=active?m.iconColor:T.textDim;return <button key={m.id} onClick={()=>setModule(m.id)} style={{background:"none",border:"none",cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:3,padding:"6px 12px",minWidth:70}}>
        <span style={{fontSize:18,color:ic,textShadow:active?`0 0 12px ${m.iconColor}`:""}}>{m.icon}</span>
        <span style={{fontSize:8,color:active?m.iconColor:"#333",fontWeight:active?700:400,letterSpacing:0.5,textTransform:"uppercase"}}>{m.label}</span>
        {active&&<div style={{width:16,height:2,background:m.iconColor,borderRadius:2,boxShadow:`0 0 6px ${m.iconColor}`}}/>}
      </button>;})}
    </div>
  </div>;
}
